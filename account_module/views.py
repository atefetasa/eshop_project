    ...
    def post(self, request):
        ...
        if register_form.is_valid():
            ...
            if user:
                ...
                new_user = User(email=user_email, email_active_code=get_random_string(10), is_active=False, username=user_email)
        ...
        if login_form.is_valid():
            ...
            if user:
                if not user.is_active:
                    login_form.add_error('email', 'حاب کاربری شما فعال نشده است')
                else:
                    ...
                    if is_password_correct:
                        ...
                        login_form.add_error('email', 'نام کاربری و یا رمز عبور اشتباه است.')
            else:
...
class ActivateAccountView(View):
    def get(self, request, email_active_code):
        ...
        if user:
            if not user.is_active:
                user.is_active =True
                user.email_active_code = get_random_string(10)
                user.save()
                return redirect(reverse('login_page'))
            else:
                pass
